...
def main(args, outs):
    ...
    with open(outs.analyze_sc_variants,'w') as cells_per_snp:
        ...
        calls_iter_proc = subprocess.Popen(['tabix',args.sc_calls,args.chrom+":"+str(args.start)+"-"+str(args.end)],stdout=subprocess.PIPE)
        calls_iter = iter(proc.stdout.readline,'')
        variant_pair_iter = pair_iter(calls, vcfreader, lambda x: (x.split("\t")[0], int(x.split("\t")[1]),x.split("\t")[4]), lambda x: (x.CHROM, x.POS, x.ALT[0]))
